...
@DynamicUpdate
@NamedQueries(value= {@NamedQuery(name="products.ID",query="select stu from products stu where stu.id=?1"),
@NamedQuery(name="products.retriveall",query="select stu from products stu order by stu.name"),
@NamedQuery(name="products.discount",query="select stu from products stu where stu.discount >= ?1 ")
})
